import React from "react";
import { Link } from "react-router-dom";
import "./style.css";
import Main from "./main";
import Footer from "../footer";
const Samarkand = () => {
  return (
    <>
      <Main />
      <div className="cotainer">
        {" "}
        <div className="more-comp-bottom">
          {/* <div class="vertical-menu">
            <Link to={"#"} class="active">
              Home
            </Link>
            <Link to={"#"}>Link 1</Link>
            <Link to={"#"}>Link 2</Link>
            <Link to={"#"}>Link 3</Link>
            <Link to={"#"}>Link 4</Link>
            <Link to={"#"}>Link 5</Link>
            <Link to={"#"}>Link 6</Link>
            <Link to={"#"}>Link 7</Link>
            <Link to={"#"}>Link 8</Link>
            <Link to={"#"}>Link 9</Link>
          </div> */}
          <div className="more-comp-right">
            <h1>Day Tour to Samarkand </h1>
            <div className="img-more">
              <img
                src="https://azizguide.com/thumb/2/BALD5LceAYsoAPXKU6fbSQ/90r90/d/afrosiab_speed_train.jpg"
                alt=""
              />
              <img
                src="https://azizguide.com/thumb/2/9qC84iUZD3nbIrMTQ5MeSQ/90r90/d/20170420_222333.jpg"
                alt=""
              />
              <img
                src="https://azizguide.com/thumb/2/YZiHc-Zxh5oTpJZrZcUyRQ/90r90/d/samarkand_sher-dor_madrassah_2.jpg"
                alt=""
              />
            </div>
            <div className="more-travel-about">
              <p>
                Welcome to magnificent Samarkand - the most ancient city of
                Uzbekistan and Central Asia. Samarkand is one of the greatest
                Silk Route's crossroad and melting pot of the world's culture.{" "}
                <br />
                Founded in the 7th century B. C. as ancient Afrasiab, Samarkand
                is the same age as Rome, Athens and Babylon. Samarkand was a
                capital of Tamerlane's Empire and It is one of the best
                preserved Islamic architecture city in the world.
              </p>{" "}
              <br />
              <p>
                This city had its most significant development in the Temurid
                period (14th -15th centuries). The major monuments include the
                Registan Ensemble (mosque and madrasahs), Gur-Emir Mausoleum,
                Bibi-Khanum Mosque, Shakhi-Zinda Necropolis and Ulugbek's
                Observatory.
              </p>
              <p>
                {" "}
                <br />
                The legendary Samarkand square of Registan has up to now been
                considered to be one of the main architectural sights of Central
                Asia. The monuments of Samarkand are majestic and wonderful. In
                this town one can feel the breath of history itself.
              </p>{" "}
            </div>
            <div className="img-more">
              <img
                src="https://azizguide.com/thumb/2/HoFdc60K1SFnTPjWFszfcg/90r90/d/samarkand_railway_station.jpg"
                alt=""
              />
              <img
                src="https://azizguide.com/thumb/2/1L936Py5f_Gr4VJZKs2OAw/90r90/d/samarkand_sher-dor_madrassah.jpg"
                alt=""
              />
              <img
                src="https://azizguide.com/thumb/2/ksIePqBVnLxF4BMYMxVRWw/90r90/d/samarkand_zhakhi-zinda_2019.jpg"
                alt=""
              />
              <img
                src="https://azizguide.com/thumb/2/MRNl0fGetuuZX4EIK7jx2w/90r90/d/samarkand_registan.jpg"
                alt=""
              />
            </div>{" "}
            <br />
            <br />
            <div className="more-travel-about">
              <b>One day tour from Tashkent to Samarkand by train.</b>
              <br /> <br />
              <p>
                <strong>07:15</strong>&nbsp;Transfer to the Central railway
                station.
              </p>{" "}
              <br />
              <p>
                <strong>08:00</strong>&nbsp;Departure by the high-speed train
                "Afrosiab" to Samarkand (2 h. 10 min.).
              </p>{" "}
              <br />
              <p>
                <strong>10:10</strong>&nbsp;Arrival in Samarkand. Transfer to
                sightseeing:
              </p>{" "}
              <br />
              <p>
                <ul>
                  <li>Guri-Emir Mausoleum (Tamerlane's and Temurids tombs).</li>
                  <li>
                    Registan Ensemble (the most famous cultic site not only in
                    Samarkand, but perhaps in all of Uzbekistan), Ulugbek
                    Madrasah, Sher-Dor Madrasah, Tillya-Kari Madrasah.
                  </li>
                </ul>
              </p>{" "}
              <br />
              <p>
                Old City and the Khast-Imam architectural complex:
                Kaffal-as-Shoshi Mausoleum, Barak-Khan Madrasah, Khast-Imam
                Mosque. Mui-Muborak Madrasah – the original Koran of the Caliph
                Usman.
              </p>{" "}
              <p>
                {" "}
                <br />
                <p>
                  <strong>Lunch.</strong>&nbsp;Continue the tour:
                </p>{" "}
                <br />
                <ul>
                  <li>Bibi-Khanum Mosque.</li>
                  <li>Siab Bazaar.</li>
                  <li>Ulugbek Observatory.</li>
                  <li>Shakhi-Zinda Necropolis.</li>
                </ul>
              </p>
              <br />
              <div className="img-more">
                <img
                  src="https://azizguide.com/thumb/2/U9UwPoSq_JHFt5QjKslLXw/90r90/d/tashkent_chorsu_dome.jpg"
                  alt=""
                />
                <img
                  src="https://azizguide.com/thumb/2/2b3VHH7BdQ34VuBxlp9z1Q/90r90/d/tashkent_chorsu_spices.jpg"
                  alt=""
                />
                <img
                  src="https://azizguide.com/thumb/2/QmwUaEwA_algjtIAL1a89w/90r90/d/tashkent_chorsu_bazaar.jpg"
                  alt=""
                />
                <img
                  src="https://azizguide.com/thumb/2/uWIlInONawDVuwn3WzCFtg/90r90/d/tashkent_pilaf_center.jpg"
                  alt=""
                />
              </div>{" "}
              <br />
              <p>
                <strong>16:20</strong>&nbsp;Transfer to railway station.
              </p>{" "}
              <br />
              <p>
                <strong>17:00</strong>&nbsp;Departure by the high-speed train
                "Afrosiab" to Tashkent (2 h. 10 min.).
              </p>
              <br />
              <p>
                <strong>19:10</strong>&nbsp;Arrival in Tashkent. Transfer to
                hotel. End of tour.
              </p>{" "}
              <br />
              <p>
                <strong>Price for the tour:</strong>&nbsp;
                <strong>$USD 250</strong> for 1 persons;{" "}
                <strong>$USD 400</strong> for the 2 persons;{" "}
                <strong>$USD 450</strong>&nbsp;for the 3 persons;
              </p>{" "}
              <br />
              <p>
                <strong>$USD 500</strong> for the 4 persons.
              </p>{" "}
              <br />
              <p>
                <strong>Tour price includes:</strong>
                &nbsp;Tashkent-Samarkand-Tashkent train tickets, all transfers,
                my time and service as your private guide.
              </p>{" "}
              <br />
              <p>
                <strong>Tour price excludes:</strong>&nbsp;Entrance tickets to
                the sites, photo/video shooting fees (for professional cameras
                only), lunch.
              </p>{" "}
              <br />
              <br />{" "}
              <p>
                <p>
                  <strong>
                    One day tour from Tashkent to Samarkand by car.
                  </strong>
                </p>
              </p>{" "}
              <br />
              <p>
                Sometimes it happens that there are no train tickets available.
                An excursion to Samarkand by car is the solution. Going by car
                means that you aren't tied to strict schedules and you are free
                to choose where to stop, whether to purchase melons or visit a
                cotton field.
              </p>{" "}
              <br />
              <div className="img-more">
                <img
                  src="https://azizguide.com/thumb/2/Jn3WLadPdNmAtFts742yjA/90r90/d/melons_bazaar_on_the_way_to_samarkand.jpg"
                  alt=""
                />
                <img
                  src="https://azizguide.com/thumb/2/9-Ut5Hq0HfdFbYk6kKEB6w/90r90/d/20171001_133948.jpg"
                  alt=""
                />
                <img
                  src="https://azizguide.com/thumb/2/R-YATj_hubS9pX2SjLI4Tw/90r90/d/samarkand_shakhi-zinda_2017.jpg"
                  alt=""
                />
                <img
                  src="https://azizguide.com/thumb/2/L5Y0uQm1ihiLBca1iXdm6Q/90r90/d/samarkand_shakhi-zinda_2018.jpg"
                  alt=""
                />
              </div>{" "}
              <br />
              <p>
                <strong>07:00</strong>&nbsp;Departure from Tashkent to Samarkand
                by car (4 hours).
              </p>{" "}
              <br />
              <p>
                <strong>11:00</strong>&nbsp;Arrival in Samarkand. Sightseeing
                tour:
              </p>
              <br />
              <ul>
                <li>
                  Guri-Emir Mausoleum (Tamerlane's and other Temurids’ tomb).
                </li>
                <li>
                  Registan Ensemble: (the most cultic site not only in
                  Samarkand, but perhaps in all of Uzbekistan) Ulugbek Madrasah,
                  Sher-Dor Madrasah, Tillya-Kari Madrasah.
                </li>
              </ul>{" "}
              <br />
              <p>
                <strong>Lunch.</strong>&nbsp;Continue the tour:
              </p>{" "}
              <br />
              <ul>
                <li>Bibi-Khanum Mosque</li>
                <li>Siab Bazaar</li>
                <li>Ulugbek Observatory</li>
                <li>Shakhi-Zinda Necropolis</li>
              </ul>{" "}
              <br />
              <div className="img-more">
                <img
                  src="https://azizguide.com/thumb/2/QyUlTrdbiqLdkEkCGwzk9w/90r90/d/samarkand_registan_2018.jpg"
                  alt=""
                />
                <img
                  src="https://azizguide.com/thumb/2/07U6l_cJUcpB1Ft7Ko7ONQ/90r90/d/screenshot_20190930-144338.png"
                  alt=""
                />
                <img
                  src="https://azizguide.com/thumb/2/lqFR60f_gErz0Rkvr0pFRg/90r90/d/samarkand_breakfast_in_the_jewish_block.jpg"
                  alt=""
                />
              </div>
              <br />
              <p>
                <strong>18:00</strong>&nbsp;Drive back from Samarkand to
                Tashkent (4 hours).
              </p>{" "}
              <br />
              <p>
                <strong>22:00</strong>&nbsp;Arrival in Tashkent. End of a tour.
              </p>{" "}
              <br />
              <br />{" "}
              <p>
                <strong>Price for the tour:</strong>&nbsp;
                <strong>$USD 300</strong> (1-3 person) sedan car;{" "}
                <strong>$USD 500</strong> (4-7 person) minivan,
              </p>{" "}
              <br />
              <p>
                <strong>$USD 700</strong> (8-14 people).
              </p>{" "}
              <br />
              <p>
                <strong>Tour price includes:</strong>&nbsp;Transportation by
                car/minivan, my time and service as your private guide.
              </p>{" "}
              <br />
              <p>
                <strong>Tour price excludes:</strong>&nbsp;Entrance tickets to
                the sites, photo/video shooting fees (for professional cameras
                only), lunch.
              </p>
              <br /> <br />
              <Link to={"#contact"}>Contact me</Link>
            </div>
          </div>
        </div>
      </div>
      <Footer />
    </>
  );
};

export default Samarkand;
